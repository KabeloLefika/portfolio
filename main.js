...
let isMobile2 = window.matchMedia('(max-width: 664px)').matches;
...
let projects = [
 {
    image: 'textures/Wildlife.jpg',
    url: 'https://github.com/KabeloLefika/WildLifeMiniProject',
    github: 'https://github.com/KabeloLefika/WildLifeMiniProject',
    demo: 'https://github.com/KabeloLefika/WildLifeMiniProject',
  },
  {
    image: 'textures/bigjimgrills.png',
    url: 'https://github.com/KabeloLefika/BigJimGrills',
    github: 'https://github.com/KabeloLefika/BigJimGrills',
    demo: 'https://github.com/KabeloLefika/BigJimGrills',
  },
  {
    image: 'textures/reavaya.png',
    url: 'https://github.com/KabeloLefika/reavaya_proj',
    github: 'https://github.com/KabeloLefika/reavaya_proj',
    demo: 'https://github.com/KabeloLefika/reavaya_proj',
  },
...
let defaultCameraPos, defaultCamerRot;

/*if (isMobile2) {
  defaultCameraPos = {
    x: 3.709028643133046,
    y: 2.5463638814987481,
    z: 0.0983449671971262,
  };
  defaultCamerRot = {
    x: 2.8313297556598935,
    y: 2.9383399492446749,
...
gltfLoader.load(
  ...
  function (room) {
    ...
    getSwitchboardPosition();
    // Call the function to start waiting for the objects
    waitForObjectsAndClickOnSwitchboard();
...
function waitForObjectsAndClickOnSwitchboard() {
  const checkObjectsInterval = setInterval(() => {

      if (titleText != null && subtitleText != null) {
          clearInterval(checkObjectsInterval); // Stop polling
          simulateClickOnSwitchboard(); // Call the function once both objects are available
      }
...
function loadIntroText() {
  ...
  loader.load('fonts/unione.json', function (font) {
    ...
    const titleGeo = new TextGeometry('KABELO LEFIKA', {
      font: font,
      size: 0.08,
      height: 0.01,
    });
  ...
  loader.load('fonts/helvatica.json', function (font) {
    ...
    const subTitleGeo = new TextGeometry(
      'Web Designer / Backend / Frontend developer/ Computer Scientist',
      {
        font: font,
        size: 0.018,
        height: 0,
...
function switchTheme(themeType) {
  if (themeType === 'dark') {
    ...
    if (bookCover) {
      const bookTexture = bookCover.material.map;
      if (bookTexture) {
        bookTexture.offset.set(5, 5); // Adjust brightness by changing offset
      }
    }
...
function getSwitchboardPosition() {
// Assuming 'scene' is your Three.js scene
var switchboardPosition;

// Traverse through the scene's children to find the switchboard object
scene.traverse(function(object) {
    if (object.name === 'Switch') {
        // Access the position of the switchboard object
        switchboardPosition = object.position;

        // Once you've found the switchboard, you may want to break out of the loop
        // if you're sure there's only one switchboard in the scene
        return;
    }
});

// Now 'switchboardPosition' holds the position of the switchboard object
console.log('Position of switchboard:', switchboardPosition);

}

function simulateClickOnSwitchboard() {
  const switchboardObject = scene.getObjectByName('SwitchBoard');

    // If the switchboard object exists, calculate its position on the screen
    if (switchboardObject) {
        const boundingBox = new THREE.Box3().setFromObject(switchboardObject);
      ...
      console.log('Switchboard object found in the scene.');
      

        // Simulate a mouse click event at the calculated position
        
    } else {
        console.error('Switchboard object not found in the scene.');
    }
}